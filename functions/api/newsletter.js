...
const sendEmailPostmark=async(apiKey,data)=>{
  ...
  const body={
    ...
    HtmlBody:`<strong>New signup</strong><br/>
  ...
  if(!res.ok){
    ...
    err.status=res.status; err.details=details; err.from=FROM; err.to=TO;
    throw err;
  }
};
...
export const onRequestPost=async(context)=>{
  ...
  catch(e){console.error("newsletter:invalid_json",e);return json({ok:false,error:"Invalid JSON"},400);}

  // Honeypot: silently accept + do nothing
  const honey=(data&&data.company?String(data.company):"").trim();
  if(honey)return json({ok:true},200);

  const submittedEmail=(data&&data.email?String(data.email):"").trim().toLowerCase();
  if(!submittedEmail)return json({ok:false,error:"Email is required"},400);
  if(!submittedEmail.includes("@")||submittedEmail.length>254)return json({ok:true},200);

  // Turnstile verification (silently fail)
  const token=(data&&data.turnstileToken?String(data.turnstileToken):"").trim();
  const ip=context.request.headers.get("CF-Connecting-IP")||"";
  const t=await verifyTurnstile({secret:TURNSTILE_SECRET,token,ip});
  if(!t.success){console.error("newsletter:turnstile_failed",t);return json({ok:true},200);}
